...
import assort from "../ZLR_GRAU/database/traders/5a7c2eca46aef81a7ca2145d/assort.json"; // adjust the path to your file
...
class MainLoader {
    ...
    compareAndReplace(originalItem: any, attributesToChange: any): any {
    ...
    getFilters(item: string): [any[], any[]] {
    ...
    async loadZLRGRAU(container: any): Promise<void> {
        ...
        const assortData = await jsonUtil.deserialize(
            await this.FileSystem.read(
                path.join(this.ThisModPath, modFolderName, "database", "traders", trader, "assort.json")
            )
        );
