...
//Item template file
import itemTemplate =       require("../ZLR_GRAU/templates/item_template.json");
import articleTemplate =    require("../ZLR_GRAU/templates/article_template.json");
...
class MainLoader {
    ...
    postDBLoad(container: any): void {
        ...
        const weaponConfig = this.jsonUtil.deserialize(fs.readFileSync(configPath, "utf-8"));
    ...
    loadComponetList(container: any, componentList: string[]): void {
    ...
    loadZLRGRAU(container: any): void {
        const modFolderName = "ZLR_GRAU";
        const modFullName = "ZLR_GRAU_RIFLE";
        const modPath = path.join(this.ThisModPath, modFolderName);
        ...
        this.items = this.jsonUtil.deserialize(fs.readFileSync(path.join(modPath, "database", "items.json"), "utf-8"));
        this.traders = this.jsonUtil.deserialize(fs.readFileSync(path.join(modPath, "database", "traders.json"), "utf-8"));
        //this.logger.info("here 2", "green")
        this.modConfig = this.jsonUtil.deserialize(fs.readFileSync(path.join(modPath, "config.json"), "utf-8"));
    ...
    {
        const modPath = path.join(this.ThisModPath, "ZLR_GRAU");
        const traders = this.jsonUtil.deserialize(fs.readFileSync(path.join(modPath, "database", "traders.json"), "utf-8"));
        //console.log(traders);
        ...
        for (const item in traders.traders[trader].assort.loyal_level_items) 
        {
            ...
            if (this.modConfig.lvl1Traders) this.db.traders[trader].assort.loyal_level_items[item] = 1;
            else this.db.traders[trader].assort.loyal_level_items[item] = traders.traders[trader].assort.loyal_level_items[item];
        }
    }
    ...
    private copyToFilters(itemClone: string, ID: string, enableCompats = true, enableConflicts = true): void
    {
        ...
        {
            if ( item in this.items.items ) continue;
    ...
    private addLocales(ID: string, Item?: IItem, Article?: ICustomizationItem): void
    {
        ...
        for (const localeID in this.db.locales.global) //For each possible locale/language in SPT's database
        {
            ...
            if ( Entry.locales )
            {
                ...
                if (this.modConfig.oldLocales) this.db.locales.global[localeID].templates[ID] = localeEntry;
            ...
            if ( isItem && Item.presets )
            {
                ...
                {
                    if (this.modConfig.oldLocales) this.db.locales.global[localeID].preset[preset] = {
